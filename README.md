...
7. Работа со стеком происходит только через верний элемент стека, через TOS.
...
2. В качестве аргументов команды принимают число, аргумент, метка,
...
2. ```ALU``` - ALU, которое принимает значения из TOS сначала на левый, а потом на
   ...
   * ```store``` - позволяет загрузить в буффер вывода данные из константы,
     ...
     из памяти не равен ```\0```, мы записываем полученные значения подряд
     в буффер вывода.
